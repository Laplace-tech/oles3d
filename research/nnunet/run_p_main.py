#!/usr/bin/env python3
"""Frozen protocol의 P organ×error-type adaptive 30k main runner."""
...
def main() -> None:
    """Clean cloud runtime에서 P를 30,000 updates까지 실행."""
    ...
    run_results_root = (
        persistent_results_root / "main" / f"p_seed_{arguments.seed}"
    )
    ...
    if arguments.continue_training:
        ...
        restore_candidate_state(
            ...
            extra_state_names=("_p_learning_state.json",),
        )
        ...
        print("Restored candidate/joint-state epoch:", completed_epochs, flush=True)
    else:
        if existing_checkpoints:
            raise FileExistsError("기존 P checkpoint가 있어 새 실행 중단")
        if pool_root.exists() and any(pool_root.iterdir()):
            raise FileExistsError("기존 P candidate/joint state가 있어 새 실행 중단")
    ...
    metadata = {
        ...
        "method": "P organ-wise plus error-type adaptive allocation",
        ...
        "organ_ema_decay": 0.9,
        "organ_adaptive_fraction": 0.5,
        "error_type_learning_state": "focus-organ raw error-share EMA",
        "error_type_ema_decay": 0.9,
        "error_type_adaptive_fraction": 0.5,
        "error_type_baseline": "current candidate-pool voxel fraction",
    ...
    print("Candidate/joint state:", pool_root, flush=True)
